...
"""
function mapzoom(run::Run;
                 minzoom::Integer = 0, maxzoom::Integer = 21)::Integer
...
"""
function mapprojection(run::Run;
    minzoom::Integer = 0, maxzoom::Integer = 21)
    zoom = mapzoom(run, minzoom = minzoom, maxzoom = maxzoom)
    tpz = 2^zoom
...
"""
function downloadmap(mapcoor;
    mapservice::AbstractString="google", maptype::AbstractString="terrain")
    if mapservice == "google"
        ...
        error("map service $mapservice not implemented")
    end
    temp
end
